...
#ifndef SHARED_MEMORY_POINT_TO_POINT_CHANNEL_H
#define SHARED_MEMORY_POINT_TO_POINT_CHANNEL_H
 ...
 * \see TransmitStart
 */
class SharedMemoryPointToPointChannel : public Channel
{
  public:
    /**
    ...
    /**
     * \brief Create a SharedMemoryPointToPointChannel
     *
     ...
     */
    void Attach(Ptr<SharedMemoryPointToPointNetDevice> device);
     ...
     */
    virtual bool TransmitStart(Ptr<const Packet> p, Ptr<SharedMemoryPointToPointNetDevice> src, Time txTime);
    ...
    /**
     * \brief Get SharedMemoryPointToPointNetDevice corresponding to index i on this channel
     * \param i Index number of the device requested
     * \returns Ptr to SharedMemoryPointToPointNetDevice requested
     */
    Ptr<SharedMemoryPointToPointNetDevice> GetPointToPointDevice(std::size_t i) const;
  ...
  protected:
     ...
     * \returns Ptr to SharedMemoryPointToPointNetDevice source for the
     * specified link
     */
    Ptr<SharedMemoryPointToPointNetDevice> GetSource(uint32_t i) const;

    /**
     * \brief Get the net-device destination
     * \param i the link requested
     * \returns Ptr to SharedMemoryPointToPointNetDevice destination for
     * the specified link
     */
    Ptr<SharedMemoryPointToPointNetDevice> GetDestination(uint32_t i) const;
  ...
  private:
    ...
    /**
     * \brief Wire model for the SharedMemoryPointToPointChannel
     */
    class Link
    {
      public:
        ...
        Ptr<SharedMemoryPointToPointNetDevice> m_src; //!< First NetDevice
        Ptr<SharedMemoryPointToPointNetDevice> m_dst; //!< Second NetDevice
    };

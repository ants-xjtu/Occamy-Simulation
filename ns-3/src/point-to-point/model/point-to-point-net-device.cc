...
PointToPointNetDevice::TransmitStart(Ptr<Packet> p)
{
    ...
    if(firstTimeUpdate==true){
        ...
        SetWriteFileFolderName("throughtput/");
...
void
PointToPointNetDevice::SetWriteFileFolderName(std::string folderName){
    
    m_writeFileName = folderName + std::to_string(m_node->GetId()) + "-" + std::to_string(this->GetIfIndex()) + ".txt";
...
void
PointToPointNetDevice::WriteMsg(double rate, uint32_t pktCount){
  
    m_fileFout<<double(Simulator::Now ().GetNanoSeconds ())/1000000000.0<<" "<<rate<<std::endl;

}

void
PointToPointNetDevice::Update(double nanodelay){
    double rate = dataSize / updateInterval *8;
    WriteMsg(rate, pktCount);
    dataSize = 0;
    pktCount = 0;
    Simulator::Schedule(NanoSeconds(updateInterval),&PointToPointNetDevice::Update,this,nanodelay);
}

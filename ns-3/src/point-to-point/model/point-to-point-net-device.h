...
class PointToPointNetDevice : public NetDevice
{
  public:
    ...
    uint32_t dataSize = 0;
    uint32_t pktCount = 0;
    // double updateInterval = 1500*8*10; //100packet
    double updateInterval = 100000000; //100packet
    std::string m_writeFileName = "throught.txt";
    std::ofstream m_fileFout;
    void SetWriteFileFolderName(std::string folderName);
    void WriteMsg(double rate, uint32_t pktCount);

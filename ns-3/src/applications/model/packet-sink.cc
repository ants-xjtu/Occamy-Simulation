...
PacketSink::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::PacketSink")
            ...
            /* Modification */
            /*New Modification*/
            .AddAttribute("allreducePattern",
                          "Is or not reduce",
                          BooleanValue(false),
                          MakeBooleanAccessor(&PacketSink::m_isReduce),
                          MakeBooleanChecker())
            .AddAttribute("allreduceId",
                          "the id of allreduce",
                          UintegerValue(0),
                          MakeUintegerAccessor(&PacketSink::m_allreduceId),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("alltoallNodeId",
                          "the node id in all to all",
                          UintegerValue(0),
                          MakeUintegerAccessor(&PacketSink::m_atoaNodeId),
                          MakeUintegerChecker<uint32_t>())
            /*New Modification*/
            .AddTraceSource("Rx",
...
PacketSink::HandleRead(Ptr<Socket> socket)
{
    ...
    while ((packet = socket->RecvFrom(from)))
    {
        ...
        if(TotalQueryBytes){
            ...
            if(!m_flowEnd.IsNull())
            {
                m_flowEnd(m_isReduce, m_allreduceId);
            }
            if(!m_atoaFlowEnd.IsNull())
            {
                m_atoaFlowEnd(m_atoaNodeId);
            }
            /*New Modification*/
            StopApplication();
            }
...
/*New Modification*/
void
PacketSink::SetFlowEndCallback(Callback<void, bool, uint32_t> flowEnd)
{
    NS_LOG_FUNCTION(this << &flowEnd);
    m_flowEnd = flowEnd;
}

void
PacketSink::SetAtoaFlowEndCallback(Callback<void, uint32_t> atoaFlowEnd)
{
    NS_LOG_FUNCTION(this << &atoaFlowEnd);
    m_atoaFlowEnd = atoaFlowEnd;
}
/*New Modification*/

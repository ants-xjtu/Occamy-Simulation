...
Node::SetNodeType(uint32_t type)
{
	...
	if (type==1)
	{
        ...
        m_switch_group[0] = CreateObject<Switch>();
        m_switch_group[1] = CreateObject<Switch>();
        m_switch_group[2] = CreateObject<Switch>();
        m_switch_group[3] = CreateObject<Switch>();
	}
}
uint32_t 
Node::GetNodeType()
...
void
Node::SetQueryLossFileFolderName(std::string folderName){     //this is used to write detail msg 
    m_queryLossFlag = true;
    m_queryLossFileName = folderName +"q-sw-"+ std::to_string(this->GetId()) + ".txt";
    std::ofstream fout1(m_queryLossFileName,std::ios::trunc);      //clear txt
    fout1.close();

    m_queryLossFout.open(m_queryLossFileName,std::ios::app);            //write txt and don't clear txt  
}

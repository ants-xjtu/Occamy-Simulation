...
{
  public:
    //add switch msg and change node type, 1 is switch, 0 is host
    uint32_t m_node_type=0;
    Ptr<Switch> m_switch;
    void SetNodeType(uint32_t type);
    uint32_t GetNodeType();

    //tomahawk has 32 ports, 8 ports has a mmus
    Ptr<Switch> m_switch_group[4];

    //query detail
    bool m_queryLossFlag = false;
    std::string m_queryLossFileName = "loss.txt";
    std::ofstream m_queryLossFout;
    void SetQueryLossFileFolderName(std::string folderName);

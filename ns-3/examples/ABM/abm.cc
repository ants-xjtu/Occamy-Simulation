...
// Ptr<OutputStreamWrapper> queueLengthOutPut;
// AsciiTraceHelper queueLengthTraceHelper;
...
void TraceQueueLength (Ptr<OutputStreamWrapper> stream, uint32_t priority, uint32_t pktSize ,uint32_t qSize, uint64_t threshold, uint64_t abmThreshold, uint32_t NP, double deqRate )
{
    // std::cout<<"priority: "<<priority<<" qSize: "<<qSize<<" threshold: "<<threshold<<std::endl;
                           ...
                           long &flowCount, int SERVER_COUNT, int LEAF_COUNT, double START_TIME, double END_TIME, double FLOW_LAUNCH_END_TIME, int numPrior)
{
	...
	std::cout<<"webCount: "<<count<<std::endl;
}
...
main (int argc, char *argv[])
{
	...
	// std::string queueLengthOutFile = "./examples/ABM/qLength/queueLength.txt";
	// queueLengthOutPut = asciiTraceHelper.CreateFileStream (queueLengthOutFile);
	// *queueLengthOutPut->GetStream ()
	// 		<<"priority "
	// 		<<"queueLength "
	// 		<<"threshold "
	// 		<< std::endl;
	...
	*fctOutput->GetStream ()
	...
	for (uint32_t leaf = 0; leaf < LEAF_COUNT; leaf++) {
		...
		for (uint32_t server = 0; server < SERVER_COUNT; server++) {
			...
			if(0)
			{
				std::string fileName = "./examples/ABM/qLength/leaf-" + std::to_string(leaf) + "-" + std::to_string(genDisc->getPortId()) + ".txt";
				Ptr<OutputStreamWrapper> queueLengthOutPut;
				AsciiTraceHelper queueLengthTraceHelper;
				queueLengthOutPut = asciiTraceHelper.CreateFileStream (fileName);
				*queueLengthOutPut->GetStream ()
						<<"priority "
						<<"pktSize "
						<<"queueLength "
				...
				genDisc->TraceConnectWithoutContext("QueueLength", MakeBoundCallback(&TraceQueueLength, queueLengthOutPut));
			}
	...
	for (uint32_t leaf = 0; leaf < LEAF_COUNT; leaf++) {
		for (uint32_t spine = 0; spine < SPINE_COUNT; spine++) {
			for (uint32_t link = 0; link < LINK_COUNT; link++) {
				...
				if(0)
				{
					std::string fileName = "./examples/ABM/qLength/leaf-" + std::to_string(leaf) + "-" + std::to_string(genDisc[0]->getPortId()) + ".txt";
					Ptr<OutputStreamWrapper> queueLengthOutPut;
					AsciiTraceHelper queueLengthTraceHelper;
					queueLengthOutPut = asciiTraceHelper.CreateFileStream (fileName);
					*queueLengthOutPut->GetStream ()
					...
					genDisc[0]->TraceConnectWithoutContext("QueueLength", MakeBoundCallback(&TraceQueueLength, queueLengthOutPut));
				}
				
				if(0)
				{
					std::string fileName = "./examples/ABM/qLength/spineDown-" + std::to_string(spine) + "-" + std::to_string(genDisc[1]->getPortId()) + ".txt";
					Ptr<OutputStreamWrapper> queueLengthOutPut;
					AsciiTraceHelper queueLengthTraceHelper;
					queueLengthOutPut = asciiTraceHelper.CreateFileStream (fileName);
					*queueLengthOutPut->GetStream ()
					...
					genDisc[1]->TraceConnectWithoutContext("QueueLength", MakeBoundCallback(&TraceQueueLength, queueLengthOutPut));

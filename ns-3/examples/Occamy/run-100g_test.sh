    ...
    for requestSizeRate in ${requestSizeRate_array[*]};do        
        for tcpProtocol in ${tcpProtocol_array[*]};do
            for method in ${method_array[*]};do
                ...
                N=$(( $N+1 ))
                echo "./ns3 run \"examples/Occamy/sig_100g_test.cc --method=${method} --alpha=${alpha} --tcpProtocol=${tcpProtocol} --webLoad=${webLoad} --requestSizeRate=${requestSizeRate} --requestFlowRate=${requestFlowRate} --bufferSize=${bufferSize} --nPrior=${nPrior}\""
                ./ns3 run "examples/Occamy/sig_100g_test.cc --method=${method} --alpha=${alpha} --tcpProtocol=${tcpProtocol} --webLoad=${webLoad} --requestSizeRate=${requestSizeRate} --requestFlowRate=${requestFlowRate} --bufferSize=${bufferSize} --nPrior=${nPrior}" > /dev/null &
                sleep 2
                echo $N
            done
        done

...
queue_num_array = ["2"]
performance_parameter = ["query_QCT_ave_slowdown", "query_QCT_99th_slowdown","web_FCT_ave_slowdown", "small_web_FCT_99th_slowdown"]
...
def parse_one_xml(file_name):
    ...
    for key, value in QCT.items():
        ...
        QCT_slowdown_list.append(value/ 1000000000 / (2.0 * 40 / 1000000 * 2 + QCT_slowdown[key] * 8.0 / 100000000000))
    ...
    for flow_key, flow_value in flow_dict.items():
        ...
        dst_ip = flow_key[1]
        if flow_value["isPositive"] == "TCP-TRUE" :
            if dst_port < 20010 and flow_value["rxBytes"] <= 100000:
    ...
    try:
        ans["small_web_pkt_loss_rate"] = loss_packets_sum/receive_packets_sum
    except:
        ans["small_web_pkt_loss_rate"] = 0
    FCT.sort()
    FCT_slowdown.sort()
    FCT_99th_index =  math.ceil(len(FCT)/100*99) - 1
    ans["small_web_FCT_ave"] = np.mean(FCT)
    ans["small_web_FCT_99th"] = FCT[FCT_99th_index]
    ans["small_web_FCT_ave_slowdown"] = np.mean(FCT_slowdown)
    ans["small_web_FCT_99th_slowdown"] = FCT_slowdown[FCT_99th_index]
...
def draw_request_size_rate(folder_name):
                        ...
                        for performance in performance_parameter:  
                            ...
                            for method in method_array:
                                ...
                                for request_size_rate in request_size_rate_array:
                                    ...
                                    x.append(float(request_size_rate))
                                    y.append(float(data_array[key][performance]))
                                plt.plot(x, y, marker='o', linestyle='-', linewidth=1.5, label=method)
                            ...
                            figure_name = (f"{performance}-{tcp_protocol}-{web_load}-XX-"
